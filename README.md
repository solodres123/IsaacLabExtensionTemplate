...
- Throughout the repository, the name `QRC` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of QRC (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/QRC
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/QRC/QRC/ui_extension_example.py`.
